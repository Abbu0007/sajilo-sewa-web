...
export default function UsersTableClient({
  ...
  const filtered = useMemo(() => {
    ...
    return users
      .filter((u) => u.role !== "admin") // ✅ hide admins always
  ...
  async function doDelete() {
    ...
    startTransition(async () => {
      try {
        ...
        // ✅ optional: re-fetch list to stay consistent with backend
    ...
    <div className="space-y-6">
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between text-sm text-slate-600">
            ...
            <div className="text-xs text-slate-500">
              Actions are instant (no reload needed).
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden">
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold">Profession</th>
              ...
              {filtered.map((u) => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-6 py-4 text-slate-700">{showProfession}</td>
        ...
        <div className="flex items-center justify-between px-6 py-4 text-xs text-slate-500">
          <div>{pending ? "Updating..." : "Ready"}</div>
          <button
            onClick={() =>
              startTransition(async () => {
                const data = await adminListUsers();
